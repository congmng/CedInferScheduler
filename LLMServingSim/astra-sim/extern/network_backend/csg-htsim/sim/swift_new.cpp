...
SwiftSubflowSrc::adjust_cwnd(simtime_picosec delay, SwiftAck::seq_t ackno) {
    ...
    _retransmit_cnt = 0;
    simtime_picosec target_delay = _src.targetDelay(true, *_route);
    //cout << "delay (us)" << timeAsUs(delay) << " target " << timeAsUs(target_delay) << endl;
    ...
    if (_src.plb()) {
	simtime_picosec td = _src.targetDelay(false, *_route); // target delay ignoring cwnd component
	cout << _src.nodename() << " delay " << delay << "rep td " << td << endl;
	if (delay > td) {
...
SwiftSubflowSrc::handle_ack(SwiftAck::seq_t ackno) {
	...
	if (!_in_fast_recovery) {
	    ...
	    _last_acked = ackno;
	    _dupacks = 0;
	    _src.log(SwiftLogger::SWIFT_RCV);
	    applySwiftLimits();
	    send_packets();
	    return;
	}
	...
	if (ackno >= _recoverq) { 
	    ...
	    _src.log(SwiftLogger::SWIFT_RCV_FR_END);
	    _retransmit_cnt = 0;
	    if (_can_decrease) {
	...
	_last_acked = ackno;
	_inflate += mss();
	_src.log(SwiftLogger::SWIFT_RCV_FR);
	retransmit_packet();
	applySwiftLimits();
	send_packets();
	return;
	...
	_src.log(SwiftLogger::SWIFT_RCV_DUP_FR);
	send_packets();
	return;
    }
	...
	_src.log(SwiftLogger::SWIFT_RCV_3DUPNOFR);
	return;
    }
    ...
    // are back on track
    _src.log(SwiftLogger::SWIFT_RCV_DUP_FASTXMIT);
}
...
SwiftSubflowSrc::send_packets() {
    ...
    while ((_last_acked + c >= _highest_sent + mss()) 
	   && (_highest_sent + mss() <= _src._flow_size+1)) {
	...
	if(_RFC2988_RTO_timeout == timeInf) {// RFC2988 5.1
	    _RFC2988_RTO_timeout = eventlist().now() + _rto;
	    cout << timeAsUs(eventlist().now()) << " " << _src.nodename() << " RTO at " << timeAsUs(_RFC2988_RTO_timeout) << "us" << endl;
	}
    }
}
...
SwiftSubflowSrc::send_next_packet() {
    ...
    _dsn_map[_highest_sent+1] = _src._highest_dsn_sent+1;
    
    SwiftPacket* p = SwiftPacket::newpkt(_flow, *_route, _highest_sent+1, _src._highest_dsn_sent+1, mss());
    //cout << timeAsUs(eventlist().now()) << " " << nodename() << " sent " << _highest_sent+1 << "-" << _highest_sent+_mss << endl;
    _highest_sent += mss();  
...
SwiftSubflowSrc::retransmit_packet() {
    cout << timeAsUs(eventlist().now()) << " " << _src.nodename() << " retransmit_packet " << endl;
    if (!_established){
...
SwiftSubflowSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    cout << timeAsUs(eventlist().now()) << " " << _src.nodename() << " At " << now/(double)1000000000<< " RTO " << _rto/1000000000 << " MDEV " 
...
void SwiftSubflowSrc::doNextEvent() {
	...
	_src.log(SwiftLogger::SWIFT_TIMEOUT);
...
SwiftSubflowSrc::move_path() {
    cout << timeAsUs(eventlist().now()) << " " << _src.nodename() << " td move_path\n";
    if (_src._paths.size() == 0) {
	cout << _src.nodename() << " cant move_path\n";
	return;
    }
    _path_index++;
    assert(_path_index < _src._paths.size()); // if we've moved paths so often we've run out of paths, I want to know
    _route = _src._paths[1];
}
...
: EventSource(eventlst,"swift"),  _logger(logger)
{
    ...
    _sub = new SwiftSubflowSrc(*this, pktlogger);  // just one subflow for now - FIXME!

    cout << eventlist().now() << " " << nodename() << " created, cwnd " << _sub->_swift_cwnd << endl;
}

void
SwiftSrc::log(SwiftLogger::SwiftEvent event) {
    if (_logger) 
	_logger->logSwift(*_sub, event);
}

void SwiftSrc::set_app_limit(int pktps) {
    if (_app_limited==0 && pktps){
	_sub->_swift_cwnd = _mss;
    }
    _app_limited = pktps;
    _sub->send_packets();
}
...
SwiftSrc::set_cwnd(uint32_t cwnd) {
    _sub->_swift_cwnd = cwnd;
    cout << eventlist().now() << " " << nodename() << " set_cwnd, cwnd " << _sub->_swift_cwnd << endl;
}
...
SwiftSrc::set_hdiv(double hdiv) {
    ...
    cout << eventlist().now() << " " << nodename() << " set_hdiv, hvid " << hdiv << endl;
}
...
void 
SwiftSrc::startflow() {
    _sub->_established = true; // send data from the start
    cout << eventlist().now() << " " << nodename() << " started, cwnd " << _sub->_swift_cwnd << endl;
...
SwiftSrc::connect(const Route& routeout, const Route& routeback, SwiftSink& sink, 
    ...
    _sub->_route = &routeout;

    assert(_sub->_route);
    _sink = &sink;
    _sub->_flow.id = id; // identify the packet flow with the source that generated it
...
simtime_picosec
SwiftSrc::targetDelay(bool add_fs_delay, const Route& route) {
    // note fs_delay can be negative, so don't use simtime_picosec here!
    double fs_delay = _fs_alpha/sqrt(_sub->_swift_cwnd/_mss) + _fs_beta;  // _sub->_swift_cwnd is in bytes
...
void
SwiftSrc::receivePacket(Packet& pkt) 
{
    ...
    //cout << timeAsUs(eventlist().now()) << " " << nodename() << " recvack  " << ackno << " + " << _inflate << endl;
    //cout << timeAsUs(eventlist().now()) << " " << nodename() << " highest_sent was  " << _sub->_highest_sent << endl;

    if (ackno < _sub->_last_acked) {
	...
	_flow_size = _sub->_highest_sent+_mss;
	_stop_time = 0;
    }

    if (ackno==0){
	//assert(!_sub->_established);
	_sub->_established = true;
    } else if (ackno>0 && !_sub->_established) {
	cout << "Should be _established " << ackno << endl;
	assert(false);
    }

    assert(ackno >= _sub->_last_acked);  // no dups or reordering allowed in this simple simulator

    simtime_picosec delay = eventlist().now() - ts_echo;
    _sub->adjust_cwnd(ackno, delay);
...
SwiftSink::SwiftSink() 
    : Logged("sink"), _cumulative_ack(0), _cumulative_data_ack(0), _packets(0), _buffer_logger(NULL)
{
    _nodename = "tcpsink";
}

void 
SwiftSink::connect(SwiftSrc& src, const Route& route) {
    _src = &src;
    _route = &route;
    _cumulative_ack = 0;
    //_cumulative_data_ack = 0; don't reset - will connect more than once
    _drops = 0;
}
...
SwiftSink::receivePacket(Packet& pkt) {
	...
	_cumulative_data_ack = seqno + size - 1;
...
void 
SwiftSink::send_ack(simtime_picosec ts) {
    const Route* rt = _route;
    
    SwiftAck *ack = SwiftAck::newpkt(_src->_sub->flow(), *rt, 0, _cumulative_ack, _cumulative_data_ack, ts);
...
void 
SwiftRtxTimerScanner::registerSwift(SwiftSrc &src) {
    _subflows.push_back(src._sub);
}
