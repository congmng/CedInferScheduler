...
int main(int argc, char **argv) {
    ...
    linkspeed_bps SERVICE1 = speedFromMbps((uint64_t)200000);
    ...
    for (int i = 0; i < 1; i++) {
        ...
        sinkLogger.monitorSink(swiftSnk[i]);
        // tell it the route
        routeout = new route_t();
        ...
        routein->push_back(queue[13]); 
        routein->push_back(pipe[13]);
        routein->push_back(queue[19]); 
        routein->push_back(pipe[19]);
        routein->push_back(queue[18]); 
        routein->push_back(pipe[18]);
        ...
        swiftSrc[2]->connect(*routeout,*routein,*swiftSnk[2],timeFromMs((int)(0)));
        swiftSrc[2]->set_stoptime(timeFromMs((int)125));
