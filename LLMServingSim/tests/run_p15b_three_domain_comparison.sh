#!/usr/bin/env bash
# P-15B's counterpart to run_real_qwen3_8b_three_domain_comparison.sh.
#
# Same topology, same load, same arms -- only the model differs, so the pair of
# runs is the "what changes when KV/token drops 8.9x (147,456 -> 16,960 B)" data
# point.  Step 5 of docs/P15B-profile路径B实现计划.md.
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$repo_root"

result_dir="${1:-$(mktemp -d /tmp/llmservingsim-p15b-three-domain.XXXXXX)}"
