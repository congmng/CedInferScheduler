...
def main():
    ...
    parser.add_argument('--enable-casr', action=argparse.BooleanOptionalAction, default=False,
                        help='enable CASR prefix-class observation and class-aware affinity control')
    parser.add_argument('--casr-control-interval-ms', type=float, default=100.0,
                        help='simulated-time interval between CASR control ticks in milliseconds')
    parser.add_argument('--casr-state-output', type=str, default=None,
                        help='optional JSONL path for per-tick prefix/cache snapshots; supports {run_id}')
    parser.add_argument('--casr-policy', type=str, default='builtin',
                        help='CASR policy class as package.module:Class, or builtin for the bundled solver')
    parser.add_argument('--casr-solver', choices=['greedy', 'lp'], default=None,
                        help='override casr.solver in the cluster config for this run')
    ...
    args.casr_state_output = _resolve_output_file(args.casr_state_output, args.run_id)
    ...
    if network_backend == 'analytical':
        ...
        # Some build layouts expose the executable directly under build/bin
        # (and container-mounted symlinks may retain an inaccessible target).
        # Resolve a usable binary before spawning ASTRA-Sim.
        if not os.path.isfile(binary) or not os.access(binary, os.X_OK):
            candidates = [
                os.path.join(astra_sim, "build/astra_analytical/build/bin/AstraSim_Analytical_Congestion_Unaware"),
                os.path.join(astra_sim, "build/astra_analytical/build/bin/AstraSim_Analytical_Congestion_Aware"),
            ]
            binary = next((path for path in candidates
                           if os.path.isfile(path) and os.access(path, os.X_OK)), binary)
    ...
    decode_npu_offsets = {sched.instance_id: sched.start_npu
                          for sched in schedulers if sched.pd_type == "decode"}
    decode_npu_counts = {sched.instance_id: sched.num_npus
                         for sched in schedulers if sched.pd_type == "decode"}
    for sched in schedulers:
        sched.decode_npu_offsets = decode_npu_offsets
        sched.decode_npu_counts = decode_npu_counts
    ...
    # ASTRA-Sim writes ``log/log.log`` relative to its process cwd.  The
    # repository checkout may be a read-only/container-mounted tree, so run
    # the backend from this run's private writable input directory.  All
    # configuration and workload paths passed above are absolute.
    backend_cwd = run_paths.inputs_root
    os.makedirs(os.path.join(backend_cwd, "log"), exist_ok=True)
    ...
    while True:
        ...
        if casr_controller is not None and casr_controller.due(current):
            ...
            if args.casr_state_output is not None:
                ...
                snapshot["flows"] = [flow.__dict__ for flow in casr_controller.last_flows]
