...
class Controller():
    ...
    def read_wait(self, p):
        ...
        while "Waiting" not in out[-1] and out[-1] != "Checking Non-Exited Systems ...\n":
            ...
            # ``readline`` returns an empty string at EOF.  Continuing here
            # spins forever after ASTRA-Sim exits (most often after reporting
            # a missing ET file), hiding the useful backend failure.
            if line == "":
                returncode = p.poll()
                if returncode is not None:
                    detail = p.stderr.read().strip()
                    raise RuntimeError(
                        f"ASTRA-Sim exited unexpectedly with status {returncode}"
                        + (f": {detail}" if detail else ""))
                raise RuntimeError("ASTRA-Sim stdout closed unexpectedly")
            # For debugging
            # print(line, end='')
            out.append(line)
        return out
