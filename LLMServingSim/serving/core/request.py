...
class Request:
    def __init__(self, id, model, input, output, arrival, instance_id, input_hash_ids=None,
                 output_hash_ids=None, class_id="default", prefix_id="none", is_init=True):
        self.id = id
        self.model = model
        ...
        # CASR control-plane identifiers. They use a bounded reusable prefix
...
class Batch:
    def __init__(self, batch_id, model, total_len, kv_len, q_list, k_list, num_prefill, num_decode, prefill_q_list, prefill_k_list, decode_k_list, batch_time, kv_size, evict=0, load=0, pd_kv_send_tokens=0):
        ...
        # Absolute first-NPU ID of the selected Decode instance.  None retains
        # the historical adjacent P/D placement for backward compatibility.
        self.pd_decode_npu_offset = None
        self.pd_decode_npu_count = 0
        # for attn prediction
        self.q_list = q_list
        self.k_list = k_list
