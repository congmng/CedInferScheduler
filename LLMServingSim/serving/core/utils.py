...
def get_workload(batch, hardware, instance_id=0, event=False, workload_name=None, inputs_root=None):
    ...
    # ASTRA-Sim accepts a filename *prefix* and expands it as
    # ``<prefix>.<npu-id>.et``.  The converter already writes ``llm.<id>.et``
    # inside each workload directory, so the prefix must include ``llm``.
