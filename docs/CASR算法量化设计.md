...
于是负荷 L 的总价就是曲线 **w_u · s · L² / (2K)** 的阶梯积分：块边界上精确，
块内最多高估 `w_u·s·K/(8m²)`（m=8 时 ≤ 满负荷价的 **0.39%**）。
第一单几乎免费、最后一单值一整次服务时间——这正是纯线性价格缺的那半个信号，
没有它，LP 会把所有类堆在最便宜的那一台上。

#### 3.2.1 实测行为：它在什么区间起作用（2026-09-24 现算）

**边际价阶梯**（P-15B WAN 档，`w_u=1`、`m=8`，单位 秒/单位工作量）：

| 实例 | 定价容量 K | 服务时间 s | 第 1 段 | 第 4 段 | 第 8 段（末段，无上界） |
|---|---:|---:|---:|---:|---:|
| p0（3090） | 4.89 | 0.825 s | 0.052 | 0.361 | 0.773 |
| p2（4090） | 8.25 | 0.449 s | 0.028 | 0.196 | 0.421 |
| p4（5090） | 13.58 | 0.291 s | 0.018 | 0.127 | 0.273 |

两个关键性质：

1. **它是"利用率"的函数，不是"绝对负荷"的函数**。写成闭式
   `w·s·L²/(2K) = w·(s·K)·(L/K)²/2`，在**相同利用率**下三台的价几乎一样
   （`s·K` = 4.04 / 3.70 / 3.95；实测在 util = 11% / 37% / 63% / 99% 四个点上，
   p0 与 p4 的价只差 **2.2%**，与 `s·K` 之比完全一致）——所以它**只惩罚"谁被压得满"，
   不惩罚"谁更慢"**；
   快慢差由**算力项** `w_c·P_i·w[i,k]` 承担。这是有意的分工。
2. **低利用率时它几乎不出声**：`L/K = 10%` 时整台只落在第一段，边际价是 `w·s/(2m)` ≈ 服务时间的 1.8%。
   所以它在"池子有余量、但某台被堆满"时才起作用。

**两个区间的实测**（740 类 × 16 rps，1250-token）：

| 情境 | `w_u=0` | `w_u=1` | `w_u=4` |
|---|---|---|---|
| **真实容量**（约束绑定） | `{0:0.54, 2:5.77, 4:9.69}` | 同上 | `{0:1.33, 2:4.98, 4:9.69}` |
| **容量 ×10**（两侧都放大 10×，约束不绑） | **`{4:16.0}`（100% 堆在最便宜那台）** | 仍是 `{4:16.0}` | `{2:3.48, 4:12.52}`（开始摊开） |

读法：

* 容量绑定的时候，**摊开由容量约束完成**，拥塞项加不加都不改变分配——这说明它**不是**当前矩阵里
  决定放置的那一项；
* 容量不绑的时候，`w_u=0` 会把 100% 堆在最便宜的 pair 上（这正是"慢卡变成便宜的溢出下水道"），
  拥塞项确实把它拉开，但在我们的量纲下要 **w_u≳4** 才压得过算力差
  （算力差 p4→p2 ≈ 0.16 s/单位，而 10% 利用率下的拥塞边际只有 0.018 s/单位）。
  沿 `w_u` 继续扫：**2 仍 100% 堆在 p4；4 开始摊开；8 与 16 稳定在 `{2:3.88, 4:12.12}`**
  ——再加价只是把常数抬高，分配不再变，所以 w_u=4 已经够用，不需要往大调。

**已知的边界**：末段无上界 ⇒ 超过容量的部分边际价**封顶在 `w·s·(2m−1)/(2m)`**
（m=8 时 = 0.94·w·s，即约一整次服务时间），
所以"跑在 100% 容量上"并不会被这项惩罚——这正是 6.27 要**另加 Prefill 排队项**的原因，
也是 6.22 里 SLO 二值罚在两端都失效的同一个病灶（价格在饱和处不再上升）。
调参时只需记住：`utilization_segments` 决定阶梯粗细，真正的判别力来自"利用率是否接近 1"。

**它同时被用在哪**：LP 目标（`_congestion_variables`）+ **闭式孪生** `_congestion_price`
（`plan_objective` / `_plan_load_cost` 用它给"已发布的计划"和结构反事实打分），
所以迟滞比较与 ±P 的收益判断用的是**同一个目标**，不会出现"LP 认为划算、迟滞按另一套价否掉"。
两者相等这一点由回归测试固化：`tests/test_runtime_capacity_scope.py::CongestionPriceTests`
（闭式 vs 解析曲线 vs **LP 实际目标**三方对照，2026-09-24）。
