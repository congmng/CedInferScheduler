...
| R8b | 替换真正带来端到端收益的那一档 | **待跑** | prefill 主导 + Decode 饱和：pack 3（3750 token）× 输出 ≤16 × `max_num_seqs ≤2` × 40–60 rps |
...
**还有一处未闭合**：我们尚未跑出"替换带来端到端收益"的那一档。按上面的比值，
它需要 prefill 主导 + Decode 饱和，即长 prompt（pack 3）× 短输出（≤16 token）
× 低 Decode 并发（max_num_seqs ≤2）× 高到达率（≥40–60 rps）。这是 §9 的 R8b。
