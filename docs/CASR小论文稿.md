...
| R7 | 结构弹性的措辞超出实现 | **已修（措辞）**，实验待跑 | §6.8 降级为 hot-spare reconfiguration；待补 peak-duration sweep（20/40/60/120 s）找 break-even horizon |
| R8 | 长输出（>16 token）下 Q2 可能翻转 | **待跑（优先级 2）** | output 长度扫描 16/64/128/256，画"选择本地重算的请求比例 vs 输出长度" |
| R9 | "结构性失效"归因过强（含 tie-break artifact） | **已修（措辞）**，对照待跑 | 改为"信号失去区分度 + 确定性 tie-break"；补 `load + RR tie-break` 臂 |
