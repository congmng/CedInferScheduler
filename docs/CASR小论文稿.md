...
**差距分解**（四段实测见 §6.9，含它的图）：`403 → 179.7`（2.24×）→ `65.3`
（2.75×，字节预算规划）→ `45.2`（1.44×，结构弹性）。第一段**不是**"打分函数
看错了资源"——§6.9 的 `load_rr` 对照证明它与 `kv_aware` 等效，也就是说
第一段主要是 **tie-break 修复**；第二、三段才是本文的主张。
...
本节记录本轮评审（2026-09-17）提出的问题与处理状态，供后续迭代对照。
