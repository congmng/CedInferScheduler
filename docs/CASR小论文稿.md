...
（**该数字的口径限制——基线被禁止本地重算——见 §1.3，公平主表见 §9 R4**）。
把决策规则单独交给基线（不改其路由）即可让基线自己降 **61%**，说明
**"有没有这个决策"是主要贡献**。在模拟器上我们进一步把差距拆成**四段实测**：
单纯换掉 tie-break（同一个 load 打分，只把决胜改成轮询）就拿到 2.56×
（419 s → 164 s），再叠加按字节预算的全局规划拿到 2.51×（→65 s），
最后结构弹性再拿 1.51×（→43 s）。**注意第二段不是"换个更聪明的指标"**：
我们新加的 KV 出口感知打分（`kv_aware`）与 `load_rr` 效果相当（175 vs 164 s），
所以真正值钱的是规划与结构，而不是打分函数本身。最后，我们用**真混合注意力模型**
（Zamba2-1.2B，38 层中仅 6 层带 KV）验证了两件反直觉的事：混合模型省下的 KV
只有 **0.85×**（层数少 6 倍但每层宽 4 倍），而算力型负载均衡在"Prefill 服务
时间短到 `inflight` 无法分化"的模型上会**失去区分度**；在确定性 tie-break 下
退化为严重偏斜（735 个请求全部落到同一台 Prefill，TTFT 419 s）。
...
第二个出发点是容量口径。Prefill 引擎的算力容量与其 **KV 出口**相差一个数量级，
而且这个比值**随模型变化**：对 Zamba2-1.2B（prefill 25.5 ms，KV 157 MB/请求）
是 **39 ÷ 1.66 ≈ 23.6 倍**；对 Qwen3-8B（prefill 107–152 ms，KV 184 MB/请求）
只有 **4.7–6.6 倍**（§3.2 给出逐行口径）。
...
* **C3 实证**：真机六档 + 模拟四类环境（同构、硬件异构、长 prompt、热点前缀）
  的完整对照，并**新增两个基线与三个对照实验**（`kv_aware`、`load_rr`；
  Q2 输出长度/Decode 利用率/目的地余量），用以回答"你的基线是不是只是没
  调好"以及"Q2 到底会不会翻转"（§6.1–§6.11）。
* **C4 可复现与工程结论**：把八个曾让结论失效的缺陷记录下来、关键的固化成回归
  测试（附录 C），包括一个会让所有长 prompt 实验静默失效的活锁，以及一个让
  Q2 的"动态切换"在实现上从未发生的单位错误；并给出 KV 体积—收益的三点刻度
  （§6.3），界定"存算协同值多少"。
...
在所有实例上都接近 0，决胜规则成为事实上的策略。我们把该盲点补上并
**实现成一个基线**（`kv_aware`），用来量化"改进打分函数"能拿回多少——而
§6.9 的对照给出的答案是**几乎没有**：同一打分的 `load_rr`（只换 tie-break）
就达到同样的效果。这条负结果也正是本文把主张收缩到"规划与结构"的原因。
...
| 1250-token 请求 KV | 184 MB（Qwen3-8B bf16） | 36 层 × **4 KB/token/层** × 1250 |
| 同上（Zamba2-1.2B bf16） | 157 MB | 6 层 × 16 KB/token/层 × 1250 + 32 层常数态 |
| ⇒ 每台 Prefill 的出口容量 | **1.41 req/s**（Qwen）/ **1.66 req/s**（Zamba2） | 0.26 GB/s ÷ 每请求字节 |
| 同卡算力容量（1024-token 参考点） | 6.6 req/s @4090、9.3 req/s @5090（Qwen）/ **39.2 req/s**（Zamba2 @4090） | profiler 步成本 |
| ⇒ **算力 ÷ 出口** | **4.7×（Qwen@4090）～6.6×（Qwen@5090）** / **23.6×（Zamba2@4090）** | 逐行算 |

**注意这个比值不是常数，而是随模型走的**：KV 层数少、算力轻的模型比值大
（Zamba2 约 24 倍），全注意力模型比值小（Qwen 约 5 倍）。这个差别决定了后续
所有决策的走向，也是我们把 `kv_egress_gbps` 放进 LP 容量项的原因（§4.3）。
...
`inflight` 几乎立刻归零，所有候选分数相同。**此时 tie-break 成为事实上的
策略**：当前实现按实例 id 决胜，于是 735 个请求全都钉在一台上。

**措辞要精确**：这不是"算力型信号本身错"，而是
**"当 Prefill 服务时间短到 `inflight` 无法分化时，compute-load 信号失去区分度；
在确定性 tie-break 下退化为严重偏斜"**。因此 §9 的后续工作里安排了一个
`load + RR tie-break` 对照臂——如果它消掉了 735/0 但仍然解决不了出口拥塞，
这条论证才算真正闭合。**算力越快、KV 越重，信号失效越彻底**，而这正是混合
注意力模型的方向。
...
### 4.3 Q1+Q2 统一：执行模式与流分配

这一节是全文的形式化核心。请求不是"先选一对 P/D、再决定搬不搬"，而是
**在两种执行模式里选一个**：

* **分离模式**：由 Prefill `p` 计算 prompt，KV 经 fabric 送到 Decode `d`；
* **本地模式**：请求直接交给 Decode `d`，由 `d` 自己完成 prefill 与 decode，
  **远端 P 完全不执行**（不是"P 算完再重算"）。

因此每个 class `c` 的流量要拆成两族：

| 变量 | 含义 | 占用谁 |
|---|---|---|
| `f^T_{c,p,d}` | 分离模式的流量（req/s） | P 的算力、**P 的 KV 出口**、链路字节、D 的 decode |
| `f^R_{c,d}` | 本地模式的流量（req/s） | **D 的 prefill + decode 算力** |
| `b_c` | class `c` 每请求的 KV 字节 | 只由 `f^T` 携带 |
| `w_{c,p}` | `max(0.05, 1 − min(0.95, h_{c,p}/r_c))` | **命中是按 worker 计的**，缓存是本地的 |
| `w^R_c` | `T_prefill(hw_d, L_c) / T_decode(hw_d)` | 本地 prefill 折算成"解码服务当量" |

```
min  Σ_{c,p,d} distance(p,d)·f^T_{c,p,d}
   + Σ_{c,d} w^R_c · f^R_{c,d}                       # 本地模式要付 Decode 的算力
   + penalty·(σ_p + σ_d + σ_e + σ_l)                 # 归一化后的超载

s.t. Σ_{p,d} f^T_{c,p,d} + Σ_d f^R_{c,d} = λ_c                        ∀c
     Σ_{c,d} f^T_{c,p,d} · w_{c,p}          ≤ K_p + s_p               ∀p
     Σ_{c,d} f^T_{c,p,d} · b_c              ≤ E_p + s_e               ∀p   # KV 出口
     Σ_{c,p,d : (p,d)∈l} f^T_{c,p,d} · b_c  ≤ B_l + s_l               ∀l   # 共享链路
     Σ_{c,p} f^T_{c,p,d} + Σ_c (1 + w^R_c)·f^R_{c,d} ≤ K_d + s_d      ∀d
     f^T, f^R, s ≥ 0
     σ_x = s_x / max(1, 该约束的名义容量)                              # 量纲归一
```

三处与"先路由再决定"的写法不同，也正是本文要主张的部分：

1. **只有 `f^T` 消耗 P 的算力与出口**。把"搬还是算"事后补一个 if，会让 LP 以为
   本地模式的流量也在占出口，从而系统性低估可接纳的分离流量——这是本稿上一版
   模型的实质缺陷，现已修正。
2. **`K_p` 不是算力容量，而是"算力容量与出口容量取小"**，其中出口容量 =
   `kv_egress_gbps ÷ 期望每请求字节`（期望字节取观测到的 token 中位数；
   对混合模型的窗口层同样成立）。
3. **`B_l` 让共享链路成为一等约束**：多个 P/D pair 可能共用同一段 fabric，
   这在广域部署里是常态而非例外。

**实现近似（必须声明）**：上述是**联合问题**，而当前实现用两段法求解——
（i）慢层 LP 先在 `f^T` 上求解（`f^R` 用上一 tick 观测到的本地比例代入 D 的
容量约束）；（ii）快层对每个请求用 §4.4 的判据做最终选择。也就是说这是
联合问题的一轮不动点迭代，**不是每 tick 精确重解**。残差可观测（本地比例
逐 tick 变化），§6.2 的消融给出偏离的代价上界。

求解器用 OR-Tools GLOP；不可用时自动回退到确定性 greedy（按 `λ_c` 降序、
枚举 pair 取最小代价）。输出除 flow 外还记录 objective 与四类 overflow 供审计。

### 4.4 Q2：搬还是算

**决策发生在请求到达时、任何 Prefill 执行之前**：先按 §4.3 的计划选定候选
pair `(p,d)`，再比较两条路径的**完整时间**（各自含排队），若本地更优则把请求
直接派给 `d`，远端 `p` **不执行任何计算**（实现见
`Router.route_arrived_requests`：`request.local_prefill = True`，
"no Prefill instance runs"）。这一点的意义是：本地模式不是"浪费一次 P 计算"，
而是**选择不进入分离路径**——它就是 §4.3 里的 `f^R`。

```
k = prompt_tokens / 1000

transfer_ms = same_node(p,d) ? 585·k                       # ms
                            : 48 + 1351·k                  # ms（含 48 ms 固定 RTT）
local_ms    = 1000 · T_prefill(hw_d, model, L) + queue_externality(d)   # ms
...
1. **`T_prefill` 取自 profiler，不是 `a·tokens` 的线性外推。** 本稿上一版用
   `0.093·k` 秒的线性式，在 3800-token 上会与实测差近 6 倍——因为那 2 s 里
   绝大部分是排队而不是算力（profiler 说 5090 上 Qwen3-8B 的 3800-token
   prefill 只要 **216 ms**）。现在两侧都写成"服务时间 + 排队"：搬运算
   fabric 上的字节序列化 + 出口排队，本地算 profiler 查表的
   `T_prefill(hardware, model, L)` + `queue_externality`。
2. `queue_externality` 表示"这次重算会给 D 的队列再加多少等待"，防止在 D 已经
   拥挤时继续把重算塞给它——否则"本地重算"会变成对 Decode 的自私惩罚。
   这一项也是**长输出下 Q2 会发生翻转**的机制来源（§7.2 威胁 7 与 §9 的
   crossover 实验）。

系数量级（585 / 1351 ms、`T_prefill` 各硬件各模型）都来自实测标定，因此
**同一份代码在不同部署上会给出不同答案**——这是设计意图而非缺陷
（§6.7 用同一份代码在局域网与广域下验证）。
...
```
horizon_s   = 评估时间窗
startup_s   = 45 s（实测容器启动），只有启动之后才开始产生收益
Gain        = (horizon_s − startup_s) · (base_objective − candidate_objective − holding)
Cost        = startup_cost + warm_cost + warm_bytes / link_bw
动作 ⟺ Gain − Cost > max(gain_threshold_abs, gain_threshold_rel · base_objective)
```

反事实里的容量口径与 §4.3 一致（出口受限），这是让 `+P` 触发得起来的关键
（实测：口径修正前 14878 ms → 修正后 1400 ms）。**当前实现里
`startup_s = 45 s` 已经进入收益窗口**（`effective_horizon = horizon_s − startup_s`，
且 dwell ≥ startup），而 `startup_cost = 0`——即"启动要花时间"已建模，
"启动要花钱/承担空闲成本"尚未建模；由此带来的措辞限制见 §6.8。
...
| **`kv_aware`**（本文新增） | `argmin max( (inflight+1)/cap , pending_egress_ns + push_ns )` | 把"绑定资源"补进同族路由；§6.9 用它证明"改进打分函数"本身几乎不带来收益 |
| **`load_rr`**（本文新增） | 与 `load` 同一打分，决胜改为轮询 | 用来分离"打分没有区分度"与"确定性 tie-break 放大偏斜" |
...
**差距分解**（四段实测与图见 §6.9）：本板 `403 → 179.7` → `65.3` → `45.2`；
同构 4090 板上的完整四段是 `419 → 164 → 65 → 43`。第一段**不是**"打分函数
看错了资源"——§6.9 的 `load_rr` 对照（同一打分、只换决胜）拿到同样的 164 s，
说明第一段主要是 **tie-break 修复**；第二、三段才是本文的主张。
...
2. **混合模型省下的 KV 只有 0.85×**：6/38 层带 KV，但那 6 层是 MHA
   ...
   KV"的合成档会得到 2.5× 的假象。
3. 需要强调措辞：Zamba2 上 735/0 的**直接原因**是 Prefill 服务时间短（31 ms）
   使 compute-load 信号失去区分度，而**确定性 tie-break 把它放大成偏斜**；
   因此它说明的是"这类信号在快 Prefill 下不可靠"，而不是"某个 KPI 必然崩"。
   §9 R9 安排了 `load + RR tie-break` 对照臂来分离这两个因素。
...
**实现边界（必须写明，且措辞要与 §4.5 一致）**：45 s 的启动**时间**已进入
模型——它在反事实里被写成 `effective_horizon = horizon_s − startup_s`，
所以峰值短于启动时间时收益自动为负；`dwell ≥ startup` 也实现了。
**未建模**的是启动的**货币/空闲成本**（`startup_cost = 0`）、需求预测与
额外的滞回。因此本文只能说：当前实现是
**dynamic Prefill pool activation / hot-spare reconfiguration**
（按逐 tick 反事实收益决定是否拉起备用 Prefill），**不是完整的 autoscaler**；
收益表述必须限定在"有热备 + 池子饱和"场景。要闭合这一点，需要把
cold-start cost 真正放进目标函数，并做**峰值时长扫描**（20/40/60/120 s）
找出 break-even horizon —— 这已列入 §9。

---

### 6.9 Q2 的两个对照实验（评审 R8/R9）

这一小节回答评审提出的两个问题：Q2 到底会不会翻转？735/0 是打分函数的锅还是
tie-break 的锅？

**实验一：`load` + 轮询 tie-break（R9）。** 同一块 6×4090、同一条 trace、
同一个静态池，只把决胜规则从"最小实例 id"改成轮询：

| arm | E2E 均值 | p95 | TTFT p50 | Prefill 落点 |
|---|---:|---:|---:|---|
| `load` | 418.9 s | 803.9 s | 418.2 s | **735 / 0** |
| **`load_rr`**（同一打分） | **163.7 s** | 312.5 s | 163.2 s | 368 / 367 |
...
![四段差距分解](figs/fig-waterfall.png)

![tie-break 对照](figs/fig-tiebreak.png)

**结论一**：735/0 的偏斜**基本上是 tie-break 造成的**，不是算力型打分本身的
缺陷——同一分数的 `load_rr` 拿到 2.56×，并且落到与 `kv_aware` 相同的位置
（164 vs 175 s）。因此本文**不再主张"我们的指标更好"**；正确表述是
"当 Prefill 服务时间短到 `inflight` 无法分化时，确定性 tie-break 会把等价分数
放大成严重偏斜，而只要按字节预算做全局规划，路由打分的差别就退居次要"。
`casr_lp` 相对 `load_rr` 的 2.51×，全部来自规划与结构。

**实验二：Decode 利用率的 crossover（R8）。** 先扫输出长度（受控固定为
16/64/128/256/512 token，同一 1250-token 输入分布、8 rps、6 个 Decode）：

![输出长度扫描](figs/fig-q2-output-length.png)

**结论二**：**输出长度本身推不动 Q2**——本地重算比例在五个档位上都是 100%，
Decode 的排队时延始终是 0–4 ms。原因是连续批处理给了每个 Decode 16 个槽，
6 个 Decode 的头部空间约 48 req/s，而供给只有 8 req/s。**真正决定 Q2 的是
Decode 利用率，不是输出长度。**

于是把 Decode 池缩到 2 台并固定 512-token 输出，扫到达率：

![Decode 利用率下的 Q2](figs/fig-q2-decode-load.png)

这里出现了两个必须分开写的结论：

1. **按部署标定的规则永远不会翻转。** 判据里 Decode 的排队外生性写成
   `min(cap, local_ms·ρ/(1−ρ))`，而 `cap = 3.0` 被当作**毫秒**使用——对 731 ms
   的搬运完全不起作用。实测：Decode 利用率一路升到 125%（TTFT p50 已达 9.4 s），
   本地重算比例仍是 **100%**。也就是说，**"在存与算之间动态切换"这件事在当前
   实现里是失效的**，这一条是评审怀疑、我们用实验确认的缺陷。
2. **修好之后（把 `cap` 解释为放大因子，并把生产者的出口排队计入搬运侧），
   替换才会发生**：本地比例在 Decode 饱和后从 100% 掉到 78–88%，transfer 份额
   出现，且 `casr_lp` 比 `load` 更早让路（16 rps：78.1% vs 84.4%）。

**但第三个结论更重要**：在这一档里切换**没有改善端到端时延**（20 rps 时两个
arm 都是约 11.7 s），因为此时 prefill 只占 Decode 工作量约 6%（1250-token
prefill ≈ 31–116 ms，而 512-token 解码是 ~2 s）。**替换的价值由
`T_prefill / (T_prefill + T_decode)` 决定**：只有 prefill 主导 Decode 工作量时，
把 prefill 挪走才真正腾出瓶颈资源。这条比值应当作为"什么时候该启用 Q2"的判据
写进系统，而不是无条件选择本地。

**替换到底值不值**：按上面的比值，它应当需要 prefill 主导 + Decode 接近饱和。
§6.11 就是把这一档做出来的结果——结论比预期更微妙，而且**不支持"启用替换总是
更好"**。

### 6.10 结构弹性的 break-even horizon（评审 R7）

把"峰值维持多久才值得 +P"这件事做成一条曲线：同一块 6×4090、同样的
1250-token prompt、8 req/s，只改峰值时长（20/40/60/120 s），
容器启动仍是实测的 45 s：

| 峰值时长 | `casr_lp`（静态池） | `casr_full`（允许 +P） | 收益 | Prefill 落点 |
|---|---:|---:|---:|---|
| 20 s | 14.7 s | 14.7 s | **+0.0%** | 2 台（未触发） |
| 40 s | 29.3 s | 28.2 s | **+3.7%** | 2 台 + 7 个请求到第 3 台 |
| 60 s | 43.7 s | 34.0 s | **+22.2%** | 2 台 + 61 个请求 |
| 120 s | 87.0 s | 51.2 s | **+41.1%** | 4 台（407/363/114/92） |

![结构弹性的 break-even](figs/fig-elastic-horizon.png)

**结论**：**峰值短于启动时间时收益恰好为零**（20 s 档两个 arm 逐位相同，
`+P` 没有被触发）；一旦峰值长过启动时间，收益迅速上升（40 s 的 +3.7% 来自
"启动后剩下的窗口很短"，60 s 已是 +22.2%）。这与
`effective_horizon = horizon − startup_s` 的建模一致，也把 §6.8 里那句
"弹性收益来自新增推送通道"补成了可操作的判据：
**只有当预期拥塞持续时间 > 容器启动时间时，启动一台 P 才划算。**

这一步同时修掉了 §6.8 记录的措辞问题：`startup_s` 不只是装饰——它决定了
这条曲线的形状，而 20 s 档的"零收益"就是它的直接证据。
...
把 §6.9 的机制推到"prefill 主导 + Decode 接近饱和"的工况：2500-token prompt
（pack 2，KV 280 MB/请求）、16-token 输出、**只留 1 台 Decode 且
`max_num_seqs = 2`**、16 req/s（峰值 35 s）。

容量算得清楚：本地模式下每个请求给 Decode 加 83 ms（prefill）+ 62 ms（16 步解码）
= 145 ms，两个槽位 ⇒ **实测 14.9 req/s** 的容量；搬运模式下每个请求占生产者出口
280 MB ÷ 0.26 GB/s = **1.16 s**，两台 Prefill ⇒ 约 **1.72 req/s**。供给 16 req/s
刻意落在"只有 Decode"（14.9）与"Decode + 出口"（16.6）之间——这正是替换应当
改变结果的窗口。

| 判据 | p50 | mean | p95 | 本地份额 | 本地 p50 | 被搬运者 p50 |
|---|---:|---:|---:|---:|---:|---:|
| `cap = 3`（部署标定，从不切换） | 1419 ms | 1421 ms | 2639 ms | 100% | 1419 ms | — |
| `cap = 50`（修好后会切换） | **251 ms** | 2128 ms | **16667 ms** | 89.6% | **241 ms** | **16490 ms** |

![替换的收益与代价](figs/fig-q2-switch-payoff.png)

三条结论，缺一条都会把故事讲错：

1. **替换确实生效，而且对留下的大多数极其有利**：89.6% 留在本地的请求，
   p50 从 1419 ms 降到 **241 ms（5.9×）**——Decode 的排队被打掉了。
2. **但它毁掉被搬走的那一小部分**：10.4% 被搬到 Prefill 出口的请求，
   p50 是 **16490 ms**，是本地路径的 **68 倍**。原因是目的地（280 MB/请求、
   0.26 GB/s 的生产者出口）**几乎没有余量**，1.16 s 的串行化代价叠上排队，
   比它想缓解的 Decode 排队还糟。
3. **净效果在均值与尾部都是负的**（mean 1421 → 2128 ms，p95 2639 → 16667 ms）。
   也就是说：**"让 Q2 能切换"本身不是收益，它需要一个"目的地有余量"的门槛。**

**为什么当前判据会过度搬运**：它只按生产者出口的**瞬时**积压（`pending_ns`）
给搬运侧加价，而一个被推过去的请求实际付的是"积压 + 1.16 s 串行化 + 它自己
造成的队列增长"。拿一个快照去比 4.2 s 的本地估计，就会一直推到出口利用率
约 93%，而那里尾延迟已经爆炸。**设计上的修法是把出口当成一个有拐点的资源而不是
一个可以灌满的队列**：仅在目的地利用率低于阈值（例如 ρ_egress < 0.8）时才允许
替换，即比较**系统边际代价**而不是两条路径各自的服务时间。

**对 Q2 主张的最终修正**：Q2 是在两个受限资源之间做负载再分配，**只有目的地
有余量时才值得**。在这套 fabric 与 2500-token prompt 下出口几乎没有余量——
所以**部署里那条"总是本地"的规则在这个regime 是对的**，而我们的实验价值在于
说清楚：什么时候它是对的、naive 的修法会怎么错、以及正确的门槛该建在哪里。
...
* **Q2 的替换需要目的地有余量**（§6.11）：在 2500-token × 16-token 输出、
  单 Decode 饱和的工况下，把请求搬到出口让留下者的 p50 改善 5.9×，却让被搬走
  的 10.4% 付出 68×，净值转负。因此正确判据不是"本地是否比搬运慢"，而是
  **"替换之后两条队列谁更长"**，即需要给目的地利用率设门槛。
...
7. **输出长度（对 Q2 最关键的威胁）**：所有档位 output 长度上限为 16 token。
   Q2 的本质是"用 Decode 的算力换网络"，而 16 token 的输出让 Decode 极闲，
   于是"本地重算"几乎总是划算。**长输出会持续占用 Decode，使
   `queue_externality` 迅速上升，Q2 应当出现真正的翻转点**。这正是当前最该
   补的实验：横轴 output 长度（16/64/128/256），纵轴选择本地重算的请求比例。
   在该图拿到之前，本文关于 Q2 的结论只能声明在短输出工况下成立。
8. **出口速率 0.26 GB/s 的来源**：那是当前 NixlConnector 生产端的实测值，
   而物理 fabric 约 2 GB/s。审稿人有权问"把 connector 优化到线速后问题是否
   消失"。§3.4 已给出部分回答（560 MB/请求时需要 3.8 GB/s，超过 fabric 本身），
   但仍需一组**带宽扫描**（0.25 / 0.5 / 1 / 2 / 4 / 8 GB/s）画出 crossover：
   带宽越大，CASR 越应退化为普通负载/缓存路由，而不是永远宣称占优。
...
可决策项（消融 −61%）；在模拟器上把差距拆成 **2.56× / 2.51× / 1.51×** 三段
（tie-break / 字节预算规划 / 结构弹性，逐段实测见 §6.9），
并用真混合注意力模型说明：当 Prefill 服务时间短到 `inflight` 无法分化时，
算力型负载信号会失去区分度，在确定性 tie-break 下退化为严重偏斜。
...
> **工作稿专用**：这一节是 2026-09-17 评审的逐条对照表（谁提了什么、改到哪一步、
> 还剩什么），投稿时应作为附录或直接删除。放在正文之后是为了不打乱
> §1–§8 的论文结构。

| # | 问题 | 状态 | 动作 |
|---|---|---|---|
| R1 | Q2 被写成"P 算完再重算"，会被质疑自我否定 P/D 架构 | **已修（写作）** | §4.3/§4.4 改为**执行模式选择**：本地模式下远端 P 不执行；形式化里拆成 `f^T` / `f^R` 两族。实现本来就是到达时决策（代码为证），原表述有误 |
| R2 | LP 未真正耦合 Q1/Q2（本地流量仍占 P 出口与链路） | **已修（形式化）** | §4.3 重写：只有 `f^T` 占用 P 算力、P 出口与链路字节；D 的容量同时承受 decode 与本地 prefill；并声明实现是**一轮不动点近似** |
| R3 | 公式量纲/索引错误 | **已修** | 链路约束加 `b_c` 与 `Σ_c`；slack 归一化成 `σ_x = s_x / 容量`；命中改为 per-worker `w_{c,p}`；Q2 单位统一为 ms；Q3 判据改为 `Gain − Cost > 阈值` |
| R4 | 主表口径不公平（基线被禁本地重算） | **待跑** | 在 `local_prefill=auto` 下重跑主表，并改为 `load / load+Q2 / kv_aware+Q2 / casr_lp / casr_full` 的分解主表（优先级 1） |
| R5 | 数字内部不一致（184 MB 的层宽、24× 的适用范围） | **已修** | §3.2 改为逐 (模型, 卡) 口径：Qwen 4.7–6.6×、Zamba2 23.6× |
| R6 | 本地重算用线性外推而非 profiler | **已修** | §4.4 改为 `1000·T_prefill(hw, model, L)` 查表 + 排队项；并说明 3800-token 的 2 s 主要是排队（算力仅 216 ms） |
| R7 | 结构弹性的措辞超出实现 | **已跑并修完** | §6.8 降级为 hot-spare reconfiguration；§6.10 补上 peak-duration 扫描：20 s **+0.0%**、40 s +3.7%、60 s +22.2%、120 s +41.1%——**峰值必须长过 45 s 启动时间**，这条曲线就是 `effective_horizon = horizon − startup_s` 的直接证据 |
| R8 | 长输出（>16 token）下 Q2 可能翻转 | **已跑（结论与预期不同）** | 输出长度扫描 16→512：**不翻转**（本地 100%，Decode 排队 0–4 ms），因为 6×16 槽 ≈ 48 req/s 远高于 8 rps；改扫 **Decode 利用率**后翻转出现（100% → 78–88%）。同时发现判据里的排队项按部署标定（cap=3）**永远不生效**，已修（§6.9） |
| R8b | 替换真正带来端到端收益的那一档 | **已跑，结论：不是无条件收益** | §6.11：2500-token × 16-token 输出 × 1 Decode（`max_num_seqs 2`）× 16 req/s。切换后本地 89.6% 的 p50 从 1419 → 241 ms，但被搬运的 10.4% 付 16.5 s，净 mean/p95 反而变差。**Q2 需要"目的地有余量"的门槛**，否则只是把队列换个地方 |
| R9 | "结构性失效"归因过强（含 tie-break artifact） | **已跑，归因被推翻** | `load_rr`（同一打分、轮询 tie-break）= 163.7 s，`kv_aware` = 174.7 s：735/0 基本是 tie-break 造成的。**论文不再主张"我们的指标更好"**，第一段优势改记为 tie-break 修复，第二段（2.51×）才是规划与结构 |
| R10 | 0.26 GB/s 是 connector 极限，需证明不是只优化了它 | **待跑（优先级 3）** | 带宽扫描 0.25–8 GB/s，画 CASR 优势随带宽衰减的 crossover |
...
| 7 | Q2 的"动态切换"在实现上**从未发生**（Decode 利用率到 125%、TTFT p50 已 9.4 s，本地仍 100%） | 判据的排队外生性写成 `min(cap, …)` 且 `cap = 3.0` 被当作**毫秒**用，对 731 ms 的搬运恒不起作用 | 把 `cap` 解释为放大因子、搬运侧计入生产者出口排队；修后本地比例 100% → 78–88%（§6.9） |
| 8 | 修好后切换**过度**：本地多数 p50 改善 5.9×，被搬运的 10.4% 却付 68× | 判据按目的地的**瞬时积压快照**加价，没有"目的地是否还有余量"的门槛 | 已定位，修法（目的地利用率门槛）列为后续工作（§6.11） |
