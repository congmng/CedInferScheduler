...
它属于同一族路由，只把打分换成绑定资源，就能拿回 2.24×（第 5.3 节）。
...
3. **算力型负载均衡在"算力快 + KV 重"的模型上结构性失效。** Zamba2 的 Prefill
   ...
   区分——735 个请求全钉在一台上（TTFT 419 s）。换成 `kv_aware` 2.24× 好转。
...
* 在**模拟器**上，把打分换成绑定资源只能拿回 2.24×，**再叠加按字节预算的全局
  规划与结构弹性才能拿满**（2.75× 与 1.44×）；这条拆解本身回答了
  "SOTA 路由差在哪"。
