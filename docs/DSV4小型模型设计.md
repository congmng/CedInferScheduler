...
3. **无 mHC 路径的跨卡确认**（mHC 已于 2026-09-18 决定砍掉，见 §4.1）：
   `design/dsv4_ref` 的纯 PyTorch 实现已在 4090（sm89）与 5090（sm120）上
   跑出同一份 logits 指纹，这条路不再依赖任何 Hopper kernel。判据已满足；
   剩下的只是把它接到 `memory_model` 的 KV 记账上（见
   [DSV4_10-30B跨卡设计.md](DSV4_10-30B跨卡设计.md) §3.3）。
