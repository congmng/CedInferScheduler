...
2. 按官方 config 与 vLLM 的 KV 布局算，V4-Flash 的 KV 约 **11.8 KB/token（fp8）**；
   ...
   **31.1 KB/token**、Zamba2-1.2B **~127 KB/token**。也就是说 V4 的 KV 比我们
   ...
   ⚠️ **这里是解析值（设计下界）**。真实 serving 栈上的实测见
   [DSV4_10-30B跨卡设计.md](DSV4_10-30B跨卡设计.md) §3.1：vLLM 0.29 给压缩层
   记的账**比解析值高一个数量级**，原因已定位到源码（§3.1.1）并实测出
   "自己修回来能拿 1.40× 容量"（§3.1.2）。
...
**34%**。当时的猜测是：① V3.2 除了 MLA 还有 **DSA indexer 的 per-token cache**，
我的 34.3 KB/token 没算它；② 论文比的是 Pro 而不是 Flash；③ 论文的 10% 可能
包含其 serving 实现按页/层对齐后的实测分配量。

**已结算（2026-09-18）**：③ 是主因。实测 vLLM 0.29 对 12 层混合压缩配置的
分配是 **97.6 KB/token**（解析 4.3 KB），逐层 spec 全打在
[DSV4_10-30B跨卡设计.md](DSV4_10-30B跨卡设计.md) §3.1.1：
**compressor 的状态缓存是 fp32、且 `tokens_per_state` 没传（默认 1）**，
于是引擎把 `block_size` 个状态记在 `block_size` 个 token 上。所以"10% / 2%"
应当理解为 **DeepSeek 自己推理栈的口径**，不是 vLLM 的口径，也不是架构的固有属性。
...
| mHC（超连接） | **与 KV 无关**，是训练稳定性 | **已砍（2026-09-18 决定）**，绕开 sm90+ 限制 |
...
> ⚠️ **这一节是早期的手估，已被后面接上参考实现的版本取代**：
> [DSV4_10-30B跨卡设计.md](DSV4_10-30B跨卡设计.md) §3 里有四档
> （`small` / `P-5B` / `P-15B` / `P-29B`）的**可复现数字**
> （`design/dsv4_ref/verify.py --summary-only`，并与模块树做过逐项对账）。
> 下面的 DSV4-S / DSV4-M 只作当时的量级参考，**绝对值不要引用**。
...
**推荐从 DSV4-S 起步**：~1.6B 总参 / ~0.34B 激活、KV 6.8 KB/token
（比我仓库里现有对照小 4.6–22 倍），单卡或双卡就能训起来，
而且它的"KV 已经不是瓶颈"这一点本身就是我们调度研究想验证的工况。

### 4.3 训练预算（粗算，用于判断可行性）

> 同样已被 [DSV4_10-30B跨卡设计.md](DSV4_10-30B跨卡设计.md) §5 取代
> （那边用的是参考实现报出的激活参数）。
...
2. **把完整层列表的玩具配置跑起来**（12 层，而不是 1 层），确认
   ...
   **已完成，但结论是反的**：`LLM(...)` 能启动，引擎报的 `KV cache size`
   **和公式差一个数量级**（12 层 97.6 KB/token vs 解析 4.3 KB）。
   归因与"改回来能拿多少"见
   [DSV4_10-30B跨卡设计.md](DSV4_10-30B跨卡设计.md) §3.1 / §3.1.1 / §3.1.2。
3. **无 mHC 路径的跨卡确认**（mHC 已于 2026-09-18 决定砍掉，见 §4.1）：
   `design/dsv4_ref` 的纯 PyTorch 实现已在 **A100（sm80）/ 3090（sm86）/
   4090（sm89）/ 5090（sm120）** 四类卡上跑出同一份 logits 指纹（fp32 相对差
   4.0e-7 ~ 9.9e-7），这条路不再依赖任何 Hopper kernel。**判据已满足**；
   缺失的两个算子也已用 Triton 写在四类卡上跑通（同上一份文档 §2.3.1）；
   模拟器侧的 KV 记账见该文 §3.3。
...
5. （若决定自训）先做 **DSV4-S 的 1–10B token 继续预训练**，同时准备 fp8
   量化管线；训练侧**没有 mHC**，稳定性由 SwiGLU clamping + anticipatory
   routing + 保守 warmup 承担（§4.1）。

---

## 7. 未决问题与风险

1. **mHC 的取舍（已拍板：砍掉）**：2026-09-18 决定不实现 mHC，标准 pre-norm
   残差替代（§4.1）。理由是它只占 33.8M 参数 / 0.3% FLOPs，却把官方路径
   锁死在 sm90+，而我们要求 3090/4090/A100 都能跑。**剩余风险**：长上下文
   训练稳定性需要靠 SwiGLU clamping 等手段自己兜，建议只在 5090 上留一个
   "加回 mHC"的对照实验，不作为前提。
2. **fp8 只在特定目的下才需要**：vLLM 的官方 V4 路径对 fp8 权重与 KV 是硬要求，
   但那条路我们跑不了（§3 硬约束 1/2），**自研版本默认 bf16**（四类卡都稳）。
   fp8 只在两个地方有用：① 24GB 卡单卡装 P-15B（14.87 GB vs 29.73 GB）；
   ② 想跑官方 checkpoint。后者还需要在加载期做 dequant（见
   [DSV4_10-30B跨卡设计.md](DSV4_10-30B跨卡设计.md) §2.3 第 4 条）。
