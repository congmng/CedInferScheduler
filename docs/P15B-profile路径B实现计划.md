...
| 层类型 | RTX3090 (sm86) | RTX4090 (sm89) | RTX5090 (sm120) | A100 (sm80) |
|---|---:|---:|---:|---:|
| r0（无状态分支，对照） | **1.000** | **1.000** | **1.000** | **1.001** |
| r4（CSA，top-k 最多 4096 状态） | **2.433** | 2.069 | 2.068 | 2.174 |
| r128（HCA） | 1.508 | 1.486 | 1.525 | 1.533 |

四张架构（sm80/86/89/120）上的比值几乎一模一样——**说明这不是某张卡的编译/调优问题，
而是 kernel 结构本身**；r0 的 1.000 则是"路径确实只在有状态分支时才被调用"的对照。
...
是同一个物理原因。四域的 bundle 都在：`profiler/perf/<HW>/casr/P15B/bf16/tp2`
（A100 另有 `tp4`）。
...
| tp2（两域与三域**同值**） | **340.27 ms** | 324.14 ms（**−4.8%**） | **12.31** → 11.60 ms |

（三域 tp2 用 `casr_p15b_three_domain_tp2.json` 跑，8 请求的负载全部落在同两个实例上，
所以数字与两域版逐位相同——这本身也说明 tp2 的 bundle 在两条链路里取的是同一份数据。）

**TP=2 买到约 16% 的绝对延迟与 14% 的 TPOT，同时把调度能加的份额从 −7.5% 压到 −4.8%**
——和 KV 压缩那条是同一个方向的结论：硬件/切分越强，"调度增益"的绝对值越小。

**采集完成（2026-09-23 05:20）**：四域的 `tp2` 与 `bf16-triton` 全部入库，
A100 另有 `tp4`；triton 对照在四张架构上一致（r0 1.000，r4 2.07–2.43，r128 1.49–1.53）。
